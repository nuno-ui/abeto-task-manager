...
// Map owner names to team slugs
const ownerToTeamSlug = {
  ...
  'Finance': 'finance',
  'Legal': 'legal',
  'QA/Developer': 'technology',
  'UX/Product': 'product',
  'Operations/QA': 'operations',
  'Product/COO': 'operations',
};
...
async function seedTasks() {
  ...
  // First, delete existing tasks to avoid duplicates
  ...
  for (const [projectSlug, tasks] of Object.entries(allProjectTasks)) {
    ...
    for (const task of tasks) {
      totalTasks++;
      const teamSlug = ownerToTeamSlug[task.owner] || 'operations';
      const teamId = teamMap[teamSlug];
      ...
      const taskData = {
        ...
        phase: task.phase,
        status: 'not_started',
        priority: task.is_critical_path ? 'high' : 'medium',
  ...
  console.log(`\nTotal tasks in database: ${count || allTasks?.length || 0}`);
}
